 ...
 */
async function invokeCustomActionEntries(entries, customModule, recognitionResults) {
 ...
 *   { localRecognitionActions, localRecognitionActionFunctions, localRegularActionFunctions }
...
export function startRecognitionPipeline({
    ...
    const primaryActions = Array.isArray(localRecognitionActions) && localRecognitionActions.length
        ? localRecognitionActions
        : localRecognitionActionFunctions;
    const normalizedPrimary = primaryActions.map(normalizeActionEntry).filter(Boolean);
        ...
        try {
            ...
            if (normalizedPrimary.length && recognitionResults.length) {
                ...
                await invokeCustomActionEntries(
                    normalizedPrimary,
                    customModules.localRecognitionActions || customModules.localRecognitionActionFunctions,
                    recognitionResults
                );
            }
        } catch (error) {
