...
**`startRecognitionLoop`** — як у **`factory/web/script.js`**: динамічний **`import()`**, guard **`recognitionRunning`**, **`videoToReusableCanvas`**, **`scaleDetectionsToVideo`**. Гілки з **`boundingBoxes`** / **`localRecognitionActions`** часто закоментовані — **`console.log`**.
