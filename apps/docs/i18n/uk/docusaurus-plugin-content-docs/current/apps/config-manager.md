...
**Config Manager** — **консоль оператора** для файлів у **`config/public`**: список профілів, перегляд сирого вмісту, **видалення** через API сервера.
...
- **Шапка** — заголовок і підзаголовок з згадкою `config/public`.
...
**Результат:** вміст `config/public` **знаходять і підтримують** без SSH.
...
| `showConfigSource(fileName)` | async | **`GET /config/public/<file>`** як **текст**. |
...
**Залежності:** **`configuration-server.js`**; статика **`/config/public`** для перегляду.
