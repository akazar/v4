...
3. Реєструється **статичний хостинг** для HTML/CSS/JS з `apps/`, кореня репозиторію (спільні `config/` і `lib/`), `factory/web`, UI kit і зібраного сайту документації за **`/documentation/`**, якщо існує **`apps/docs/build`**.
