...
export function parseIceServersString(raw) {
    ...
    try {
        ...
        if (typeof process !== 'undefined' && process?.env && s.length > 0) {
            console.warn(
                '[ice-servers] ICE_SERVERS is not valid JSON; using default STUN only. ' +
                    'Use a JSON array with double-quoted keys, e.g. [{"urls":"turn:example.com:3478","username":"u","credential":"p"}]'
            );
        }
