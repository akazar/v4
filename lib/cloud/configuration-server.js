 ...
 * `config/public/assets/<configId>/`. Custom action JS files are ES modules with
...
const ASSET_FILE_NAMES = Object.freeze({
    ...
    localRecognitionActionFunctions: 'localRecognitionActionFunctions.js',
...
function assetUrlForId(id, fileName) {
    return `/config/public/assets/${encodeURIComponent(id)}/${fileName}`;
}
...
export function setupConfigurationServer(app) {
    ...
    app.post('/api/configurations/:id/assets', async (req, res) => {
        try {
            ...
            const ALLOWED_EXTRA_PREFIXES = ['lib/', 'config/'];
            const savedExtras = {};
            const skippedExtras = {};
            if (extraFiles) {
                for (const [rawRel, value] of Object.entries(extraFiles)) {
                    ...
                    await fs.writeFile(filePath, value, 'utf8');
                    savedExtras[rel] = `/config/public/assets/${encodeURIComponent(id)}/${rel}`;
                }
            }

            const ALLOWED_COPY_PREFIXES = ['lib/', 'config/'];
            ...
            if (copyFiles) {
                for (const [rawSrc, rawDest] of Object.entries(copyFiles)) {
                    ...
                    copied[rawSrc] = `/config/public/assets/${encodeURIComponent(id)}/${dest}`;
                }
            }
