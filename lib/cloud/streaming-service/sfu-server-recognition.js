...
import { createServerRecognitionActionsManager } from '../pipeline/server-pipeline.js';
...
/**
 * @param {string} configPath - URL path e.g. /db/configs/public/config-dashboard.js
 */
...
export function registerSfuServerRecognition(io, deps) {
  ...
  io.on('connection', (socket) => {
    socket.on('sfu-server-recognition-subscribe', async ({ streamId, configPath, configName, config }) => {
      ...
      try {
        ...
        const cfg = config && typeof config === 'object'
          ? config
          : await loadPublicStreamConfig(configPath);
        if (resolvedConfigName) {
          if (config && typeof config === 'object' && typeof scheduledActionsManager.registerConfig === 'function') {
            scheduledActionsManager.registerConfig(streamId, resolvedConfigName, cfg);
          } else {
