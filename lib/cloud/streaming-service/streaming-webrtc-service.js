...
import { getIceServersForNode } from '../../ice-servers.js';
import { registerSfuStreamKeysGetter } from './streaming-registry.js';
import {
  registerSfuServerRecognition,
  stopSfuServerRecognitionForStream,
  notifySfuPublisherMediaUpdated,
} from './sfu-server-recognition.js';
 ...
 */
export function setupStreamingWebRTCService(io, deps) {
  const { getViewerSet, emitAvailableStreams } = deps;
  const iceServers = getIceServersForNode();
  ...
  registerSfuServerRecognition(io, {
    getSession: (streamId) => sessions.get(streamId),
    wrtc: RTCPeerConnection ? wrtc : null,
  });
  ...
  function cleanupSession(streamId) {
    stopSfuServerRecognitionForStream(streamId);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('sfu-publisher-offer', async ({ streamId, offer }) => {
      ...
      publisherPc.ontrack = (event) => {
        ...
        if (ms) {
          ...
          notifySfuPublisherMediaUpdated(streamId);
        }
      };
    ...
    socket.on('sfu-unregister-streamer', ({ streamId }) => {
      if (!streamId || socket.data.sfuStreamId !== streamId) return;
      const s = sessions.get(streamId);
      if (!s || s.publisherSocketId !== socket.id) return;
      const viewers = getViewerSet(streamId);
      if (viewers) {
        for (const viewerSocketId of viewers) {
      ...
      cleanupSession(streamId);
      delete socket.data.sfuRole;
      delete socket.data.sfuStreamId;
      emitAvailableStreams();
    });
