import { orchestrateServerActions } from './cloud/action-services/server-actions-orchestrator.js';
...
export function createScheduledActionsManager(options = {}) {
  ...
  const fallbackActionProperties = Array.isArray(options.fallbackActionProperties)
    ...
    : ['localRecognitionActions', 'serverRecognitionActionFunctions'];
  ...
  function registerConfig(streamId, configName, config) {
    if (!streamId || typeof streamId !== 'string') {
      throw new Error('registerConfig(streamId, configName, config): streamId must be a non-empty string');
    }
    const trimmedStreamId = streamId.trim();
    const trimmedConfigName = String(configName || config?.id || config?.name || 'inline-config').trim();
    if (!trimmedStreamId || !trimmedConfigName) {
      throw new Error('registerConfig(streamId, configName, config): streamId/configName cannot be empty');
    }
    const normalized = normalizeConfig(config || {});
    configCache.set(trimmedConfigName, normalized);
    streamStates.set(trimmedStreamId, {
      configName: trimmedConfigName,
      config: normalized,
      lastProcessAt: Date.now(),
