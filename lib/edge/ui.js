...
export function injectTopButtons(doc, config) {
    ...
    if (config.name) {
        const nameBar = doc.createElement('div');
        nameBar.className = 'edge-ui-name-bar';
        nameBar.textContent = config.name;
        nameBar.style.cssText = [
            ...
            'padding-top:max(clamp(4px, 1vw, 8px), env(safe-area-inset-top, 0px))',
