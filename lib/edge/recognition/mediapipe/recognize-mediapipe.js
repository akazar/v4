...
const TASKS_MODULE = new URL('./models/vision_bundle.mjs', import.meta.url).href;
const WASM_BASE = new URL('./models/wasm/', import.meta.url).href;
const MODEL_ASSET_PATH = new URL('./models/efficientdet_lite2.tflite', import.meta.url).href;

let detector = null;

/**
 * Initialize MediaPipe Object Detector (local models under ./models/).
 * @param {Object} detectorOptions - MediaPipe ObjectDetector options
 * @returns {Promise<ObjectDetector>}
 */
async function initDetector(detectorOptions = {}) {
  const mediapipeModule = await import(TASKS_MODULE);
 ...
 * @param {Object} config - Recognition config (localRecognition.threshold, localRecognition.maxResults, localRecognition.classes)
...
async function recognize(source, config = {}) {
  ...
  if (!detector) {
    const detectorOptions = {
      ...
      scoreThreshold: config.localRecognition.threshold,
      maxResults: config.localRecognition.maxResults || 10,
    };
  ...
  return processDetections(rawDetections, {
    classes: config.localRecognition.classes,
