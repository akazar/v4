...
function attachSocket(socket, serverUrl) {
    ...
    // When hosted same-origin as the v4 server, `serverUrl` may be omitted and `window.io()`
    // will default to `location.origin`. When hosted elsewhere (e.g. `npx serve` on :8080,
    // file://, or another machine), `serverUrl` MUST point at the v4 server explicitly —
    // otherwise the socket connects to the static-file host where there is no signaling
    // server, so SFU publishing silently never reaches the v4 SFU.
    return { socket: serverUrl ? window.io(serverUrl) : window.io(), ownsSocket: true };
}
 ...
 * @param {object} [params.socket] - Existing socket.io-client socket. If omitted, one is created via `window.io(serverUrl)`.
 * @param {string} [params.serverUrl] - Signaling server URL. If omitted, `window.io()` defaults to the page origin.
 *   Must be set when the page is hosted somewhere other than the v4 server (e.g. `npx serve`, file://, a CDN).
...
export function createWebRtcPublisher({
    ...
    const { socket, ownsSocket } = attachSocket(providedSocket, serverUrl);
    ...
    function replaceVideoTrack(newVideoTrack) {
        ...
        const replaceInPc = (pc) => {
            if (!pc) return;
            const sender = pc.getSenders().find((s) => s.track?.kind === 'video');
            if (sender) void sender.replaceTrack(newVideoTrack);
        };
        replaceInPc(sfuPublisherPc);
        for (const pc of peerConnections.values()) replaceInPc(pc);
        const oldVideo = mediaStream.getVideoTracks()[0];
        if (oldVideo && oldVideo !== newVideoTrack) {
            mediaStream.removeTrack(oldVideo);
            try { oldVideo.stop(); } catch { /* ignore */ }
        }
        if (!mediaStream.getVideoTracks().includes(newVideoTrack)) {
    ...
    return { stop, registerStreamer, replaceVideoTrack };
}
